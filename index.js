...
// 스캔 시간을 15초로 증가 (매트의 광고 주기가 길 수 있음을 대비)
...
class HeatingMatAccessory {
    ...
    async initializeBleAdapter() {
        try {
            ...
            if (!isPowered) {
                this.log.warn(`[BLE] 어댑터(${this.adapterId})가 꺼져 있습니다. 전원을 켭니다.`);
    ...
    async startScanningLoop() {
        ...
        // Mac 주소 로깅을 강화하여 어떤 주소를 찾고 있는지 명확히 표시
        const targetAddress = this.macAddress.toUpperCase();
        this.log.info(`[BLE] 백그라운드 스캔/연결 루프 시작. (대상 MAC: ${targetAddress})`);
        ...
        while (this.isScanningLoopActive) {
            ...
            try {
                const isPowered = await this.adapter.isPowered();
                if (!isPowered) {
                    this.log.error(`[BLE] 어댑터(${this.adapterId})가 꺼져 있어 스캔 불가. 15초 후 재시도.`);
                    await sleep(this.scanInterval);
                    continue;
                }
                ...
                this.log.error(`[BLE] 장치 검색/연결 실패: ${errorMessage}. ${this.scanInterval / 1000}초 후 재시도...`);

                // 스캔/연결 오류 발생 시 장치를 초기화하고 재시도
                this.disconnectDevice(`스캔 루프 오류: ${errorMessage}`, true);
                if (this.isScanning) {
