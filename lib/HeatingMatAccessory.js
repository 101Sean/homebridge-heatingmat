...
class HeatingMatAccessory {
    ...
    async discoverCharacteristics() {
        try {
            ...
            try {
                const disService = await gatt.getPrimaryService(this.serviceDisUuid);
                const fwChar = await disService.getCharacteristic(this.charFirmwareUuid);
                await fwChar.readValue();
                this.log.debug('[BLE] 펌웨어 정보 동기화 완료');
            } catch (e) {
                this.log.debug('[BLE] 펌웨어 정보 읽기 건너뜀');
            }
    ...
    handleNotification(data, type) {
        this.log.info(`[Notify Raw Packet] ${type}: ${data.toString('hex')}`);

        const value = parsePacket(data);
        if (value === 0 || value === 255) return; // 0, 255 패킷 차단
        const finalValue = value > 12 ? (255 - value) & 0xFF : value; // notify 응답 역산

        this.log.debug(`[Notify Raw] ${type}: ${value} (Converted: ${finalValue})`);

        if (type === 'temp') {
            const targetTemp = LEVEL_TEMP_MAP[finalValue];
            if (targetTemp) {
                ...
                this.log.info(`[Sync] 온도 동기화: ${targetTemp}°C (레벨 ${finalValue})`);
            }
        } else if (type === 'timer') {
            if (finalValue <= MAX_TIMER_HOURS) {
                this.currentState.timerHours = finalValue;
                this.currentState.timerOn = finalValue > 0;

                const brightness = finalValue * BRIGHTNESS_PER_HOUR;

                this.timerService.updateCharacteristic(this.Characteristic.On, this.currentState.timerOn);
                this.timerService.updateCharacteristic(this.Characteristic.Brightness, brightness);

                this.log.info(`[Sync] 타이머 동기화: ${finalValue}시간 (${Math.round(brightness)}%)`);
            }
        }
    }
    ...
    async sendTimerCommand(hours) {
        const packet = createControlPacket(hours);
